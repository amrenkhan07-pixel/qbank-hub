...
const canonicalBatchQualityMigration = readFileSync(resolve(root, 'supabase/migrations/20260913130000_canonical_batch_quality_review.sql'), 'utf8');
const canonicalBatchReviewSecurityMigration = readFileSync(resolve(root, 'supabase/migrations/20260914100000_canonical_batch_review_save_security.sql'), 'utf8');
const canonicalBatchQualityValidation = readFileSync(resolve(root, 'scripts/canonical-batch-quality-validation.sql'), 'utf8');
...
check('taxonomy_review.server_pagination_is_bounded', taxonomyReviewMigration.includes('least(greatest(p_page_size,25),50)') && (taxonomyBrowserSource.includes("db.rpc('qbank_taxonomy_review_page'") || taxonomyBrowserSource.includes("db.rpc('qbank_canonical_batch_review_page'")) && !taxonomyBrowserSource.includes('p_limit:380'));
...
check('taxonomy_review.quick_navigation_installed', ['next-unreviewed','next-low','next-unresolved','next-negative','previous-question','next-question'].every((id) => taxonomyBrowserHtml.includes(id)));
...
check('taxonomy_v3.review_ui_shows_concept_and_evidence', taxonomyBrowserSource.includes("['Canonical Concept'") && (taxonomyBrowserSource.includes("['Positive evidence used'") || taxonomyBrowserSource.includes("['Explanation evidence used'")) && taxonomyBrowserSource.includes('proposed_concept_path'));
...
check('canonical_batch_quality.explicit_negative_intent', canonicalBatchClassifier.includes('NEGATIVE_STEM') && canonicalBatchClassifier.includes('"intent_state": 1 if row["polarity"] == "negative" else 0') && canonicalBatchQualityMigration.includes('intent_state in (0,1)'));
check('canonical_batch_quality.negative_options_are_conditional', canonicalBatchClassifier.includes('"options_for_negative": bool(selected and selected["options_score"])') && canonicalBatchClassifier.includes('* (1.4 if negative else 0)'));
check('canonical_batch_quality.correct_answer_is_evidence_not_identity', canonicalBatchClassifier.includes('correct_answer_text') && canonicalBatchClassifier.includes('positive_explanation_text') && canonicalBatchClassifier.includes('stem_text') && canonicalBatchClassifier.includes('source_test_prior_used'));
check('canonical_batch_quality.reclassification_is_exact_and_reversible', canonicalBatchQualityMigration.includes('qbank_reclassify_canonical_batch_v2') && canonicalBatchQualityMigration.includes('Reclassification IDs differ from the committed 1000-question cohort') && canonicalBatchQualityMigration.includes('set is_current=false'));
check('canonical_batch_quality.review_page_is_server_bounded', canonicalBatchQualityMigration.includes('least(greatest(p_page_size,25),50)') && taxonomyBrowserSource.includes("db.rpc('qbank_canonical_batch_review_page'") && taxonomyBrowserSource.includes('pageSize:25'));
check('canonical_batch_quality.question_content_is_lazy', taxonomyBrowserSource.includes("db.functions.invoke('taxonomy-review-question'") && taxonomyBrowserSource.includes('contentCache') && !taxonomyBrowserSource.includes('select(\'question_html'));
check('canonical_batch_quality.lazy_detail_accepts_exact_batch', taxonomyQuestionFunction.includes("db.rpc(\"qbank_canonical_batch_review_page\"") && taxonomyQuestionFunction.includes('Number(batchMembership.data?.total || 0) === 1'));
check('canonical_batch_quality.review_filters_and_navigation', ['sample-subject','sample-confidence','sample-status','sample-negative','sample-override','sample-pyq','sample-review','next-unreviewed','next-low','next-unresolved','next-negative'].every((id) => taxonomyBrowserHtml.includes(`id="${id}"`)));
check('canonical_batch_quality.review_correction_persists_separately', taxonomyBrowserSource.includes("db.rpc('qbank_save_canonical_batch_review'") && canonicalBatchQualityMigration.includes("review_scope='batch-1000-v2'") && canonicalBatchQualityMigration.includes('corrected_concept_id'));
check('canonical_batch_quality.review_save_has_narrow_authenticated_gateway', canonicalBatchReviewSecurityMigration.includes('security definer') && canonicalBatchReviewSecurityMigration.includes('from public,anon') && canonicalBatchReviewSecurityMigration.includes('to authenticated'));
check('canonical_batch_quality.rapid_navigation_is_guarded', taxonomyBrowserSource.includes('detailRequest=++state.detailRequest') && taxonomyBrowserSource.includes('if(detailRequest!==state.detailRequest)return'));
check('canonical_batch_quality.live_validation_present', canonicalBatchQualityValidation.includes('quality.exact_cohort_1000') && canonicalBatchQualityValidation.includes('quality.no_content_copied') && canonicalBatchQualityValidation.includes('learner.state_unchanged'));
