...
with v as (
  ...
  union all select 'taxonomy.review_rpc_is_bounded',case when jsonb_array_length(public.qbank_taxonomy_review('canonical-medical-v1')->'nodes')=(select count(*) from public.canonical_taxonomy_nodes n join v on v.id=n.taxonomy_version_id) then 0 else 1 end,'Review payload contains taxonomy nodes only'
