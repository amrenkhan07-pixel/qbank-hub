...
export function readClock(clock, now=Date.now(), limit=Infinity) {
 const delta=clock.paused?0:Math.min(Math.max(0,(clock.questionLimitMs ?? 50000)-clock.questionUsedMs),Math.max(0,now-clock.startedAt),Math.max(0,limit-clock.totalUsedMs));
...
export function checkpointClock(clock,now=Date.now(),limit=Infinity){const next=readClock(clock,now,limit);return {...next,paused:clock.paused||next.questionUsedMs>=(clock.questionLimitMs ?? 50000)||next.totalUsedMs>=limit,startedAt:now,savedAt:now};}
