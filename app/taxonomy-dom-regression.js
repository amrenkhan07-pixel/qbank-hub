...
async function choose(form, choices) {
  ...
  await pause(1200);
  if (form.__cascadeReady) await form.__cascadeReady;
}
...
export async function runTaxonomyDomRegression({ form }) {
  ...
  const prepTopicCount = visible(form, 'topics').length;
  const prepSubtopicCount = visible(form, 'subtopics').length;
  const prepSelected = selected(form);
  const prepValidSourceIds = new Set(form.__lastFacets?.source_tests || []);
  const prepExtraSources = prepSourceRows.filter(({ input }) => !prepValidSourceIds.has(input.value)).map(({ label }) => label);
  cases.prepladderCanonicalSubject = prepSelected.platforms.length === 1 && prepSelected.subjects.length === 1;
  cases.sourceTestsSeparateFromTaxonomy = same(prepSourceRows.map(({ input }) => input.value), prepValidSourceIds)
    && prepTopicCount === 0 && prepSubtopicCount === 0;
  ...
  return {
    ...
    evidence: {
      ...
      prepLadderAnaesthesia: { sourceTests: prepSourceRows.length, validSourceTests: prepValidSourceIds.size, extraSources: prepExtraSources, topics: prepTopicCount, subtopics: prepSubtopicCount, selected: prepSelected },
    },
  };
}
