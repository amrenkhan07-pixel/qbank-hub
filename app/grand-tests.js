import {loadCompletedTaxonomy,renderTaxonomy} from './gt-taxonomy-analytics.js';
...
export function createGrandTests({ db, e, richHtml, safeUrl, decodePayloadObject, layout, startGT }) {
  const timeUsed = a => { const end=a.paused_at?Date.parse(a.paused_at):a.completed_at?Date.parse(a.completed_at):Date.now(); const minutes=Math.max(0,Math.floor((end-Date.parse(a.started_at)-Number(a.paused_duration_ms||0))/60000)); return `${minutes} min elapsed`; };
  let catalogPromise;
  let generation = 0;
