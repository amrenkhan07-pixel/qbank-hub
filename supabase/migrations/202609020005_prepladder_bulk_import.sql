-- Generalize the validated PrepLadder two-phase importer to the canonical 19-subject catalog.
-- Source tests remain provenance; no topic/subtopic or exam metadata is fabricated.

select pg_advisory_xact_lock(hashtext('prepladder_bulk_import_v1'));
...
declare
  ...
  v_allowed constant text[] := array[
    'Anaesthesia','Anatomy','Biochemistry','Community Medicine','Dermatology','ENT',
    'Forensic Medicine','Medicine','Microbiology','Obstetrics & Gynecology','Ophthalmology',
    'Orthopedics','Pathology','Pediatrics','Pharmacology','Physiology','Psychiatry','Radiology','Surgery'
  ];
begin
  if current_user <> 'service_role' then raise exception 'trusted import role required'; end if;
  if p_manifest->>'platform' <> 'PrepLadder' or not (v_subject=any(v_allowed)) then
    raise exception 'subject is outside the canonical PrepLadder catalog';
  end if;
  if coalesce(p_manifest->>'source_subject','')='' then raise exception 'source subject provenance is required'; end if;
  ...
  insert into public.qbank_hybrid_import_runs(
    ...
    'PrepLadder',v_subject,p_manifest->>'importer_version',(p_manifest->>'schema_version')::integer,
...
declare
  ...
  v_subject text := p_manifest->>'subject'; v_source_subject text := p_manifest->>'source_subject';
  v_allowed constant text[] := array[
    'Anaesthesia','Anatomy','Biochemistry','Community Medicine','Dermatology','ENT',
    'Forensic Medicine','Medicine','Microbiology','Obstetrics & Gynecology','Ophthalmology',
    'Orthopedics','Pathology','Pediatrics','Pharmacology','Physiology','Psychiatry','Radiology','Surgery'
  ...
  attempts_before bigint; sessions_before bigint; state_before bigint; bookmarks_before bigint;
begin
  if current_user <> 'service_role' then raise exception 'trusted import role required'; end if;
  if p_manifest->>'platform'<>'PrepLadder' or not (v_subject=any(v_allowed)) then
    raise exception 'subject is outside the canonical PrepLadder catalog';
  end if;
  if coalesce(v_source_subject,'')='' then raise exception 'source subject provenance is required'; end if;
  ...
  where r.source_sha256=p_manifest->>'source_sha256' and r.platform='PrepLadder' and r.subject=v_subject
    and r.parser_version=p_manifest->>'importer_version' and r.schema_version=(p_manifest->>'schema_version')::integer;
  ...
  select id into v_platform_id from public.platforms where lower(btrim(name))='prepladder' limit 1;
  if v_platform_id is null then raise exception 'existing PrepLadder platform not found'; end if;
  select id into v_subject_id from public.subjects where name=v_subject limit 1;
  if v_subject_id is null then raise exception 'canonical subject not found: %',v_subject; end if;
  select id into v_platform_subject_id from public.platform_subjects
   where platform_id=v_platform_id and subject_id=v_subject_id;
  if v_platform_subject_id is null then
    insert into public.platform_subjects(platform_id,subject_id,native_label)
    values(v_platform_id,v_subject_id,v_source_subject) returning id into v_platform_subject_id;
  end if;

  for row in select value from jsonb_array_elements(p_manifest->'source_tests') loop
    if coalesce(row->'source_path'->>0,'')<>v_source_subject then
    ...
    if existing is not null and (existing->>'source_test_id'<>row->>'source_test_id'
      or existing->>'title'<>row->>'title' or existing->>'subject_id'<>v_subject_id::text) then
  ...
  if exists (
    ...
    left join storage.objects stored on stored.bucket_id='qbank-payloads' and stored.name=manifest_object->>'object_path'
    where stored.id is null or coalesce((stored.metadata->>'size')::bigint,-1)<>(manifest_object->>'stored_bytes')::bigint
  ...
  for row in select value from jsonb_array_elements(p_manifest->'versions') loop
    insert into public.questions(
      ...
      explanation_html,question_images,explanation_images,source_test_label,source_collection,
      source_type,is_pyq,content_origin,status,is_usable,unusable_reason
    ) values (
      (row->>'question_id')::uuid,v_platform_id,v_subject_id,
      (row->>'source_question_id')||'@'||left(row->>'content_sha256',12),row->>'stem_excerpt','[]'::jsonb,
      array_to_string(array(select jsonb_array_elements_text(row->'correct_option_keys')),','),null,
      '[]'::jsonb,'[]'::jsonb,row->>'first_source_test_title','PrepLadder '||v_subject,
  ...
  for row in select value from jsonb_array_elements(p_manifest->'occurrences') loop
    update public.qbank_source_occurrences set is_current=false
      where source_test_id=(row->>'source_test_id')::uuid and question_position=(row->>'question_position')::integer
  ...
  if exists (
    select 1 from public.qbank_source_tests t where t.platform_id=v_platform_id and t.subject_id=v_subject_id
  ...
  if exists (
    select 1 from public.qbank_question_payloads p left join public.qbank_payload_objects o on o.id=p.payload_object_id
  ...
  if (select count(*) from public.question_options)<>options_before
    ...
    or (select count(*) from public.user_question_state)<>state_before
    or (select count(*) from public.bookmarks)<>bookmarks_before then
...
create or replace function public.qbank_prepladder_import_stats()
returns jsonb language plpgsql stable security invoker set search_path=public,storage,pg_temp as $$
begin
  if current_user<>'service_role' then raise exception 'trusted import role required'; end if;
  return jsonb_build_object(
    'database_bytes',pg_database_size(current_database()),
    'storage_bytes',(select coalesce(sum((metadata->>'size')::bigint),0) from storage.objects where bucket_id='qbank-payloads'),
    'storage_objects',(select count(*) from storage.objects where bucket_id='qbank-payloads'),
...
revoke all on function public.qbank_prepladder_import_stats() from public,anon,authenticated;
...
grant execute on function public.qbank_prepladder_import_stats() to service_role;
